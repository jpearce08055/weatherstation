...
def voltageToDegrees(value, defaultWindDirection):
	...
	if (fuzzyCompare(3.84 * ADJUST3OR5, value)):
	        return 360.0 #changed to be able to filter out the bad readings
...
#Don't have to test - we know it's a 1115		
		config.ADS1015_Present = False
		config.ADS1115_Present = True
	...
	def current_wind_direction(self):
			...
			if IsTesting: print ('>>> Wind Value Read')
    		else:
      			# user internal A/D converter
      			voltageValue = 0.0
			if IsTesting: 
				voltageValue = 500.0
				print ('>>> Wind Value Not Read')
		#Here's where wind direction is read using the above function
